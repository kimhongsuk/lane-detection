#include "lane_detection/lane_detecter.hpp"
...
LaneDetector::LaneDetector() : Node("lane_detector") {
	subscription_ = this->create_subscription<sensor_msgs::msg::image>("front_image", 1, std::bind(&LaneDetector::topic_callback, this, _1));
        publisher_ = this->create_publisher<lane_detection::msg::lane_coef>("lane", 1);
}

cv::Mat LaneDetector::PolyFit(std::vector<int> x_value, std::vector<int> y_value) {
}

cv::Mat LaneDetector::DetectLinesSlidingWindow(const cv::Mat& frame, const bool bView) {
}

cv::Point LaneDetector::WarpPoint(const cv::Point center, const cv::Mat trans) {
}

float LaneDetector::GetLowPassFilter(const double sample_time, const float est_value, const float prev_res) {
}

cv::Mat LaneDetector::EstimateDistance(const cv::Mat frame, const cv::Mat trans, const double cycle_time, const bool bView) {
}

cv::Mat LaneDetector::EstimatePose(const cv::Mat frame, const double cycle_time, const bool bView) {
}



LaneDetector::DisplayImage() {
}
      /******* recording log *******/    
